...
Includes:
- 50,000 URL dataset generation (25k benign + 25k malicious)
...
# =============================================================================
# 1. DATASET GENERATION (50,000 URLs)
# =============================================================================
...
def create_dataset(n_benign=25000, n_malicious=25000):
...
def train_all_models():
    ...
    # Save comparison (exclude large arrays from json)
...
# =============================================================================
# MODEL PERFORMANCE SECTION (without caching to avoid hash errors)
# =============================================================================

st.markdown("---")
st.markdown("## 📊 Model Performance")

def get_test_predictions(model, feat_cols):
    df = pd.read_csv(DATASET_PATH)
    df = df.drop_duplicates(subset=['url']).dropna(subset=['url','label'])
    df['label'] = df['label'].astype(int).clip(0,1)
    X = pd.DataFrame([extract_features(u) for u in df['url']])[feat_cols].fillna(0).values.astype(float)
    y = df['label'].values
