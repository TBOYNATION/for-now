...
# =============================================================================
# SECTION 1 — AUTO-INSTALL MISSING PACKAGES + IMPORTS
# =============================================================================
# WHY AUTO-INSTALL?
# ─────────────────
# On Streamlit Cloud (and some other environments) third-party packages may
# not be pre-installed. Rather than relying on a requirements.txt file, this
# block uses pip at runtime to install any missing package before importing it.
# Standard-library modules (os, re, math, time, csv, random, urllib) are
# always available and need no installation.

import os, re, math, time, csv, random, sys, subprocess
from urllib.parse import urlparse, parse_qs

def _ensure(package: str, import_name: str = None):
    """
    Import `import_name` (defaults to `package`).
    If the import fails, install `package` via pip then retry.
    Raises RuntimeError if installation also fails.
    """
...
# Install / import third-party dependencies
_ensure("joblib")
_ensure("numpy")
_ensure("pandas")
_ensure("matplotlib")
_ensure("scikit-learn", "sklearn")
# streamlit is always present (it's what runs this file)
